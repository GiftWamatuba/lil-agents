"""Agent session management for Claude, Codex, and Copilot — Windows."""
...
# Suppress console windows when spawning CLI subprocesses.
...
class AgentProvider(Enum):
    ...
    def install_instructions(self):
        if self == AgentProvider.CLAUDE:
            return (
                "To install Claude Code, run in PowerShell:\n"
    ...
    def fallback_paths(self):
        ...
        paths = [
            ...
            os.path.join(appdata, 'npm', name + '.exe'),
            os.path.join(appdata, 'npm', name),
            os.path.join(localappdata, 'Programs', 'Claude', name + '.exe'),
        ]
        if self == AgentProvider.CLAUDE:
            paths.insert(0, os.path.join(home, '.claude', 'local', 'bin', 'claude.exe'))
...
class AgentSession:
    ...
    def _launch(self, binary_path):
        # .cmd/.bat scripts must be invoked via cmd.exe on Windows.
        if binary_path.lower().endswith(('.cmd', '.bat')):
            args = ['cmd.exe', '/c', binary_path]
        else:
            args = [binary_path]
