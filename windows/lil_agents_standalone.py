...
Optional:     pip install pystray  (for system tray quit button)
"""
...
def _resource_dir():
    ...
    if hasattr(sys, '_MEIPASS'):
        return sys._MEIPASS          # inside a PyInstaller .exe
...
CHAR_WIDTH  = 56
CHAR_HEIGHT = 98
BUBBLE_AREA = 0           # no bubbles in standalone mode
WIN_HEIGHT  = CHAR_HEIGHT
_KEY_COLOR  = '#010101'   # chroma-key transparent color
_SPRITE_FPS = 30.0
_SPRITE_CACHE: dict = {}
TICK_MS = 16              # ~60 fps
...
def _load_sprites(name):
    ...
    candidates = [
        os.path.join(base, 'sprites', key),                              # bundled in .exe
        os.path.join(os.path.dirname(__file__), 'sprites', key),         # local sprites/
        os.path.join(repo_root, '.cache', 'windows-sprites', key),       # extracted cache
        os.path.join(repo_root, '.cache', 'linux-sprites', key),         # shared cache
    ]
    frames_dir = None
    for c in candidates:
        if os.path.isdir(c):
...
class CharacterWindow:
    ...
    def _render(self):
        ...
        # Composite RGBA frame onto chroma-key background.
        out = PILImage.new('RGB', (CHAR_WIDTH, WIN_HEIGHT), (1, 1, 1))
        r, g, b, a = frame.split()
        out.paste(frame.convert('RGB'), (0, 0), mask=a)

        photo       = ImageTk.PhotoImage(out)
        self._photo = photo   # keep reference to prevent GC
...
class LilAgentsApp:
    ...
    def _setup_tray(self):
        if not HAS_TRAY:
            print('[lil agents] Running without system tray (pip install pystray to add it).')
